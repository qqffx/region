...
unsigned int buffer[DMA_BUFFER_SIZE] __attribute__((section(".orbita_buffer"))) = {0};
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(!stopFlag) {
      if(((DMA_BUFFER_SIZE - DMA1_Stream0->NDTR) > DMA_BUFFER_SIZE/2) &&
         (circ_buffer_ptr < DMA_BUFFER_SIZE/2)) {
        for(int i = 0; i < 2; ++i) {
          CDC_Transmit_FS(buffer + circ_buffer_ptr,
          ...
          if(debugIndex < debugBufferLength) {
            debugBuffer[debugIndex].NDTR = DMA1_Stream0->NDTR;
            debugBuffer[debugIndex].dmaFlag = dmaFlag;
            debugBuffer[debugIndex].StartBufferEntry = 
              *(unsigned int *)(buffer + circ_buffer_ptr);
            debugIndex++;
            
            // calculate new position of circular buffer pointer
...
static void my_DMA_init(void) {
  ...
  DMA1_Stream0->NDTR = DMA_BUFFER_SIZE - 1;
...
static void printDebugBuffer(void) {
  ...
  for(int i = 0; i < debugBufferLength; ++i) {
    ...
    printf("NDTR: %u dmaFlag: %u StartBufferEntry %10u \r\n",
           debugBuffer[i].NDTR,
